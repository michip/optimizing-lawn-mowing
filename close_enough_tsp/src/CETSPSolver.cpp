...
CETSPSolver::CETSPSolver(std::vector<Point> &points,
            Point &start_point,
            double radius,
            double time, double ub) : CETSPSolver(points, radius, time, ub) {
    this->points.insert(this->points.begin(), start_point);
    this->radii.insert(this->radii.begin(), 0.0);
    this->demands.insert(this->demands.begin(), 1.0);
}

CETSPSolver::CETSPSolver(std::vector<Point> &points,
                         double radius,
                         double time, double ub) :
        points(points) {
...
CETSPSolver::solution CETSPSolver::solve() {

    int selectingRoot = ROOT_SELECTION_1;
    int branchingRule = BRANCHING_RULE_SB;
    int branchingStrategy = BRANCHING_STRATEGY_BeFS;
    ...
    //### root node selection initial_strategy ###
    if (selectingRoot == ROOT_SELECTION_1) root->pts = bnbPtr->selectRoot();
    if (selectingRoot == ROOT_SELECTION_2) root->pts = bnbPtr->selectRoot2();
    if (selectingRoot == ROOT_SELECTION_3) root->pts = bnbPtr->selectRoot3();
    ...
    while (!open.empty() && cpuTime() - initialTotalTimeBnB <= timeLimit) {
        ...
        if (current->notCovered.empty() || current->lb > ub) {
            ...
            for (std::size_t t = 0; t < (std::size_t) strongBranchingSize && t < current->notCovered.size(); t++) {
                ...
                for (std::size_t i = 0; i < current->pts.size(); i++) {
...
std::vector<Point> CETSPSolver::convertSolution(std::vector<std::vector<double>> &solution) {
